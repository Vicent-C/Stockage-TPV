...
$username = $_SESSION['Usuario'];
    ...
    $consulta = "SELECT albaran.num_albaran, albaran.fecha_entrada, lineas_albaran.kilos, lineas_albaran.precio_kilo, albaran.proveedor 
    FROM albaran 
    JOIN lineas_albaran ON albaran.num_albaran = lineas_albaran.albaran WHERE lineas_albaran.kilos > 0 GROUP BY lineas_albaran.albaran";
    ...
    <section class="container-fluid col-lg-10 col-md-10 col-sm-10">
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Albaranes</li>
          </ol>
        </nav>
        <article class="col-12">
            <h4 class="display-6 text-center">Albaranes</h4>
        </article>
        ...
        <!--    Selector de tipo de orden   -->
            <div id="verfruta" class="d-flex flex-column container my-3 shadow bg-danger-subtle btn border border-secondary-subtle">
            <a href="abastecer.php" class="align-self-end btn btn-success my-3"><i class="fas fa-plus"></i> Nuevo pedido</a>
              <div class="table-responsive">
                  <table class="table table-striped table-danger table-hover">
                    <thead class="text-muted">
                        <th class="text-center">Nº Albarán</th>
                        ...
                        <?php
                          ...
                          if($stmt1->num_rows>0){
                            ...
                            while($row = $stmt1->fetch_array()){  
                              ...
                              $consultaSuma = "SELECT SUM(lineas_albaran.kilos * lineas_albaran.precio_kilo) AS total
                                               FROM lineas_albaran
                                               WHERE lineas_albaran.albaran = " . $row['num_albaran'];
                              ...
                              $consultaSumaCantidades = "SELECT SUM(lineas_albaran.kilos) AS total_cantidades
                              FROM lineas_albaran
                              WHERE lineas_albaran.albaran = " . $row['num_albaran'];
                              ...
                              echo "<tr>";
                              echo     "<td>" . $row['num_albaran'] . "</td>"; 
                              echo     "<td>" . $row['proveedor'] . "</td>";
                              echo     "<td>" . $row['fecha_entrada'] . "</td>";
                              echo     "<td>" . $cantidadUnid . " kg</td>";
                              echo     "<td>" . round($rowSuma['total'], 2) . " €</td>";
                              echo     "<td><a id='' href='./includes/veralbaran.php?albaran=" . $row['num_albaran'] . "' role='button' class='btn btn-outline-secondary' data-toggle='modal'><i class='fa-sharp fa-solid fa-circle-info'></i></a></td>";
                              echo "</tr>";
                            }
                          
                          }else{
                            echo "<tr>";
                            echo "<td colspan='6'>No se encontraron albaranes</td>";
                            echo "<tr>";
                          }
                         ?>
                </tbody>
